...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run tests with resource monitoring")
    parser.add_argument(
        "--type",
    ...
    parser.add_argument(
        "--python-version", default="python3", help="Python executable to use"
    )
    parser.add_argument(
    ...
    base_cmd = [args.python_version, "-m", "pytest", "tests/unit", "-v"]
    ...
    output_file = (
        f"logs/resource_monitor_{test_name.lower().replace(' ', '_')}_{timestamp}.jsonl"
    )
