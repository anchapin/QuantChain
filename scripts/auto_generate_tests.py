...
def main():
    ...
    parser.add_argument(
        "--top", type=int, default=5, help="Generate tests for top N low-coverage files"
    )
    parser.add_argument(
        "--all", action="store_true", help="Generate tests for all low-coverage files"
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    for i, file_info in enumerate(
        low_coverage[: args.top] if not args.all else low_coverage, 1
    ):
