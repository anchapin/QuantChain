...
class CoverageMonitor:
    ...
    def get_improvement_trend(self, entries: int = 5) -> List[float]:
        ...
        recent = (
            self.history[-entries:] if len(self.history) >= entries else self.history
        )
        return [entry["total_coverage"] for entry in recent]

    def analyze_low_coverage_files(
        self, coverage_data: Dict, threshold: float = 50
    ) -> List[Dict]:
                ...
                low_coverage.append(
                    {
                        ...
                        "total_lines": file_data.get("summary", {}).get(
                            "num_statements", 0
                        ),
