...
class TestFillModel:
    ...
    def test_immediate_fill_market_order(self):
    ...
    def test_immediate_fill_limit_order(self):
        """Test ImmediateFill model for limit orders."""
        fill_model = ImmediateFill()
        
        # Buy limit order - fill when price <= limit
        buy_order = OrderRequest("AAPL", OrderSide.BUY, OrderType.LIMIT, 100, price=150.0)
        ...
        sell_order = OrderRequest("AAPL", OrderSide.SELL, OrderType.LIMIT, 100, price=150.0)
    ...
    def test_immediate_fill_stop_order(self):
        """Test ImmediateFill model for stop orders."""
        fill_model = ImmediateFill()
        
        # Buy stop order - fill when price >= stop
        buy_stop = OrderRequest("AAPL", OrderSide.BUY, OrderType.STOP, 100, stop_price=155.0)
        ...
        sell_stop = OrderRequest("AAPL", OrderSide.SELL, OrderType.STOP, 100, stop_price=145.0)
...
class TestPerformanceMetrics:
    ...
    def test_update_metrics_multiple_trades(self):
        """Test updating metrics with multiple trades."""
        metrics = PerformanceMetrics()
        equity_curve = [100000, 105000, 98000, 102000]
...
class TestPaperTradingExecutor:
    ...
    def test_place_buy_order(self, executor):
        ...
        expected_cost = result.avg_fill_price * 100 + 1.0 + (0.01 * 100)  # price + commission
    ...
    def test_insufficient_position(self, executor):
    ...
    def test_limit_order(self, executor):
        """Test limit order behavior."""
        executor.set_market_price("AAPL", 150.0)
        
        # Limit buy below market - should fill
        buy_order = OrderRequest("AAPL", OrderSide.BUY, OrderType.LIMIT, 100, price=149.0)
        result = executor.place_order(buy_order)
        assert result.status == OrderStatus.FILLED
        
        # Limit buy above market - should not fill
        executor.set_market_price("AAPL", 150.0)
        buy_order2 = OrderRequest("AAPL", OrderSide.BUY, OrderType.LIMIT, 100, price=151.0)
        result2 = executor.place_order(buy_order2)
        assert result2.status == OrderStatus.PENDING
    
    def test_stop_order(self, executor):
    ...
    def test_cancel_order(self, executor):
        ...
        limit_order = OrderRequest("AAPL", OrderSide.BUY, OrderType.LIMIT, 100, price=140.0)
    ...
    def test_get_account(self, executor):
        ...
        assert account.portfolio_value >= 99000.0  # Portfolio includes position value (accounting for commissions)
    ...
    def test_performance_metrics_tracking(self, executor):
    ...
    def test_set_market_price(self, executor):
    ...
    def test_update_market_data(self, executor):
    ...
    def test_export_trade_history(self, executor):
    ...
    def test_is_market_open(self, executor):
