...
class TestDashboardCharts:
    ...
    def test_create_performance_chart_with_plotly(self):
        ...
        returns = pd.Series([0.01, 0.02, -0.01, 0.03], index=pd.date_range('2023-01-01', periods=4))
...
class TestWebDashboardApp:
    ...
    def test_load_portfolio_data(self):
        ...
        mock_portfolio_data = {
            'positions': [
                {'symbol': 'AAPL', 'quantity': 100, 'avg_cost': 150.0, 'current_price': 155.0},
                {'symbol': 'GOOGL', 'quantity': 50, 'avg_cost': 2000.0, 'current_price': 2100.0}
            ],
            'cash': 10000.0,
    ...
    def test_calculate_portfolio_metrics(self):
        ...
        portfolio_data = {
            'positions': [
                {'symbol': 'AAPL', 'quantity': 100, 'avg_cost': 150.0, 'current_price': 155.0},
                {'symbol': 'GOOGL', 'quantity': 50, 'avg_cost': 2000.0, 'current_price': 1900.0}
            ],
            'cash': 10000.0
        }
...
class TestWebDashboardIntegration:
    ...
    def test_full_workflow(self):
        ...
        with patch('quantchain.tools.web_dashboard.HAS_STREAMLIT', True):
            with patch('quantchain.tools.web_dashboard.HAS_PLOTLY', True):
                with patch('quantchain.tools.web_dashboard.st', mock_st):
                    with patch('builtins.open', mock_open_read(json.dumps(portfolio_data))):
                        with patch('os.path.exists', return_value=True):
                            app = WebDashboardApp()
        ...
        with patch('builtins.open') as mock_open:
            ...
            with patch('json.dump') as mock_dump:
                with patch('quantchain.tools.web_dashboard.HAS_STREAMLIT', True):
                    ...
                    mock_dump.assert_called_once_with(portfolio_data, mock_file, indent=2)
