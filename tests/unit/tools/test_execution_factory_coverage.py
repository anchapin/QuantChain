...
class TestExecutionFactoryCoverage:
    ...
    def test_factory_error_handling(self):
        ...
        error_scenarios = [
            ...
            lambda: (setattr(config, 'broker_type', None), create_execution_interface(config)),
            # Invalid broker type
            lambda: (setattr(config, 'broker_type', 'nonexistent'), create_execution_interface(config)),
            # Missing credentials (should raise AuthenticationError)
            lambda: (setattr(config, 'broker_type', 'alpaca'), create_execution_interface(config)),
        ]
