...
from quantchain.backtesting.engine import BacktestConfig, BacktestEngine, BacktestResult, MetricsResult
...
class MockBacktestEngine(BacktestEngine):
    ...
    def run(
        ...
        self._results = BacktestResult(
            ...
            start_date=config.start_date or datetime(2024, 1, 1, tzinfo=timezone.utc),
            end_date=config.end_date or datetime(2024, 12, 31, tzinfo=timezone.utc),
            initial_cash=config.initial_cash,
            final_cash=config.initial_cash * 1.1,  # 10% return
            total_return=0.10,
            ...
            trades=pd.DataFrame(),
            daily_returns=pd.Series(np.random.randn(len(data)) * 0.01, index=data.index),
        )
...
class TestBacktestResult:
    ...
    def test_backtest_result_with_minimum_values(self) -> None:
        ...
        assert result.strategy_name == "MinimalStrategy"
...
class TestBacktestEngine:
    ...
    def test_mock_implementation(self) -> None:
        ...
        assert results.final_cash == 110000.0
