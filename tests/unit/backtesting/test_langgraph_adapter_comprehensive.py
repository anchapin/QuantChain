...
class TestLangGraphAdapter:
    ...
    @pytest.mark.skipif(not PANDAS_AVAILABLE, reason="Pandas dependencies not available")
    ...
    async def test_async_backtesting_execution(self):
        ...
        for i in range(25):  # Process 25 bars
            state = await execute_backtest_step(initial_state.copy() if i == 0 else processed_states[-1])
        ...
        signal_count = sum(1 for state in processed_states if state['current_signal'] is not None)
        assert signal_count == 2  # Should have signals at bars 10 and 20

        # Test signal validity
        signals_with_actions = [state['current_signal'] for state in processed_states
    ...
    def test_portfolio_simulation(self):
        ...
        if pd is not None:
            ...
            for trade in trades:
                ...
                if trade['action'] == 'BUY':
                    ...
                    if trade['symbol'] not in portfolio['positions']:
                        portfolio['positions'][trade['symbol']] = {'quantity': 0, 'avg_price': 0}
            ...
            assert portfolio['cash'] < initial_portfolio['cash']  # Cash decreased due to purchases
    ...
    def test_performance_metrics_calculation(self):
        ...
        total_return = (backtest_results['final_capital'] - backtest_results['initial_capital']) / backtest_results['initial_capital']
        win_rate = backtest_results['winning_trades'] / backtest_results['total_trades']

        # Test basic metrics
        assert total_return == 0.25  # 25% return
        assert win_rate == 0.628  # 62.8% win rate
        ...
        if NUMPY_AVAILABLE and volatility > 0:
            sharpe_ratio = (avg_daily_return * 252 - risk_free_rate) / (volatility * np.sqrt(252))
    ...
    def test_multi_strategy_backtesting(self):
        ...
        total_weight = sum(config['weight'] for config in strategies.values() if config['enabled'])
        ...
        if total_confidence > 0:
            normalized_votes = {action: weight / total_confidence for action, weight in vote_weights.items()}
