...
def temp_env_file():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".env", delete=False) as f:
        ...
        f.write('ALPACA_API_SECRET="testsecret12345678901234567890123456789012345678"\n')
...
def env_manager(temp_env_file):
    ...
    with patch.dict(os.environ, {}, clear=True):  # Clear env vars to avoid loading system variables
        manager = EnvSecretManager(env_file=temp_env_file, allow_production_warning=False)
        yield manager
...
class TestEnvSecretManager:
    ...
    def test_init_without_env_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {}, clear=True):  # Clear env vars to avoid loading system variables
                manager = EnvSecretManager(env_file=str(non_existent), allow_production_warning=False)
    ...
    def test_get_secret_existing(self, env_manager):
        ...
        assert env_manager.get_secret("ALPACA_API_SECRET") == "testsecret12345678901234567890123456789012345678"
    ...
    def test_get_service_credentials(self, env_manager):
        ...
        assert alpaca_creds["secret"] == "testsecret12345678901234567890123456789012345678"
    ...
    def test_validate_credentials_alpaca(self, env_manager):
        ...
        # Test invalid format
        assert env_manager.validate_credentials("alpaca", "invalid_key", "invalid_secret") is False
    ...
    def test_validate_credentials_polygon(self, env_manager):
        ...
        assert env_manager.validate_credentials("polygon", "test_polygon_key_12345") is True
    ...
    def test_set_api_key(self, env_manager):
        ...
        env_manager.set_api_key("alpaca", "ANEWKEY12345678901", "newsecret1234567890123456789012345678901234567890")
    ...
    def test_load_credentials_invalid_env_file(self):
        ...
        try:
            with patch.dict(os.environ, {}, clear=True):  # Clear env vars to avoid loading system variables
                manager = EnvSecretManager(env_file=temp_path, allow_production_warning=False)
                # Should load valid lines despite invalid ones
                assert not manager._credentials  # No valid service patterns in this test
        finally:
            if os.path.exists(temp_path):
                os.unlink(temp_path)
