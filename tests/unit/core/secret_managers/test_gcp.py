...
def test_init_with_service_account_dict(self) -> None:
        ...
        with patch(
            ...
            with patch("google.oauth2.service_account.Credentials") as mock_credentials:
                ...
                manager = GCPSecretManager(
                    project_id="test-project", service_account_key=service_account_key
                )
        ...
        try:
            with patch(
                ...
                with patch(
                    ...
                    manager = GCPSecretManager(
                        project_id="test-project", credentials_path=temp_path
                    )
...
def test_build_secret_name(self) -> None:
        ...
        with patch(
            ...
            assert (
                manager._build_secret_name("test")
                == "projects/test-project/secrets/quantchain/test"
            )
            assert (
                manager._build_secret_name("quantchain/test")
                == "projects/test-project/secrets/quantchain/test"
            )
            assert (
                manager._build_secret_name("/test")
...
def test_build_secret_version_name(self) -> None:
        ...
        with patch(
            ...
            assert (
                manager._build_secret_version_name("test")
                == "projects/test-project/secrets/quantchain/test/versions/latest"
            )

            # Test specific version
            assert (
                manager._build_secret_version_name("test", "2")
...
def test_get_secret_with_quantchain_prefix(self) -> None:
        ...
        with patch(
            ...
            mock_instance.access_secret_version.assert_called_with(
...
def test_get_secret_not_found(self) -> None:
        ...
        with patch(
            ...
            mock_instance = Mock()
            mock_instance.list_secrets.return_value = Mock()
            mock_instance.access_secret_version.side_effect = gcp_exceptions.NotFound(
                "Secret not found"
            )
            mock_client.return_value = mock_instance
...
def test_validate_service_not_found(self) -> None:
        ...
        with patch(
            ...
            mock_instance = Mock()
            mock_instance.list_secrets.return_value = Mock()
            mock_instance.get_secret.side_effect = gcp_exceptions.NotFound(
                "Secret not found"
            )
            mock_client.return_value = mock_instance
