    ...
    def test_get_tool_count(self):
        ...
        with patch('quantchain.core.agent_engine.create_llm_provider') as mock_llm:
            ...
            with patch('quantchain.core.agent_engine.StateGraph'):
                try:
                    agent = QuantChainAgent()
                    count = agent.get_tool_count() if hasattr(agent, 'get_tool_count') else 0
    ...
    def test_has_tool(self):
        ...
        with patch('quantchain.core.agent_engine.create_llm_provider') as mock_llm:
            ...
            with patch('quantchain.core.agent_engine.StateGraph'):
                try:
                    agent = QuantChainAgent()
                    result = agent.has_tool("test_tool") if hasattr(agent, 'has_tool') else False
    ...
    def test_get_tool_names(self):
        ...
        with patch('quantchain.core.agent_engine.create_llm_provider') as mock_llm:
            ...
            with patch('quantchain.core.agent_engine.StateGraph'):
                try:
                    agent = QuantChainAgent()
                    names = agent.get_tool_names() if hasattr(agent, 'get_tool_names') else []
