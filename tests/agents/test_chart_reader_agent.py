...
class TestTechnicalIndicatorCalculator:
    ...
    def sample_ohlcv(self):
        ...
        ohlcv_data = {
            'timestamps': [datetime.now() - timedelta(hours=i) for i in range(num_points)][::-1],
            'opens': [prices[i] * (1 + np.random.normal(0, 0.001)) for i in range(num_points)],
            'closes': prices[:num_points],
            'highs': [prices[i] * (1 + abs(np.random.normal(0, 0.005))) for i in range(num_points)],
            'lows': [prices[i] * (1 - abs(np.random.normal(0, 0.005))) for i in range(num_points)],
...
class TestPatternRecognizer:
    ...
    def test_analyze_chart_vision_model_failure(
        ...
        pattern_recognizer.llm_provider.generate_vision.side_effect = Exception("Vision model failed")
