"""Training mode for learning-focused trading simulation."""
        ...
        if self.rag_system:
            try:
                ...
                for data in relevant_data:
                    ...
                    elif data.data_type == "news" and symbol in data.content.get("symbols", []):
...
class MistakeTracker:
    ...
    def analyze_mistake(self, order: OrderResult, market_context: Dict[str, Any]) -> Optional[TradingMistake]:
        ...
        # Check for risk management mistakes
            ...
            mistake = self._create_mistake(order, "risk_management", "critical", market_context)
        
        # Check for market misread mistakes
        elif self._is_market_misread(order, market_context):
            mistake = self._create_mistake(order, "market_misread", "minor", market_context)
        
        if mistake:
            self.mistakes.append(mistake)
            self.mistake_patterns[mistake.mistake_type] = self.mistake_patterns.get(mistake.mistake_type, 0) + 1
    ...
    def _create_mistake(self, order: OrderResult, mistake_type: str, severity: str, 
                       context: Dict[str, Any]) -> TradingMistake:
        ...
        for mistake_type, count in self.mistake_patterns.items():
            if count >= 3:  # Pattern detected after 3 similar mistakes
                if mistake_type == "timing":
                    recommendations.append("Focus on market timing - study volume and volatility patterns")
                elif mistake_type == "sizing":
                    recommendations.append("Review position sizing strategy - implement fixed percentage rules")
                elif mistake_type == "risk_management":
                    recommendations.append("Strengthen risk management - add stop-losses and position limits")
                elif mistake_type == "market_misread":
                    recommendations.append("Improve market analysis - use multiple confirmation signals")
...
class ConfidenceMetrics:
    ...
    def calculate_confidence_score(self) -> float:
        ...
        weights = [0.7 + (0.3 * i / len(self.decisions)) for i in range(len(self.decisions))]
        ...
        weighted_score = sum(q * w for q, w in zip(quality_scores, weights)) / sum(weights)
    ...
    def calculate_consistency_score(self) -> float:
        ...
        consistency = max(0.0, 1.0 - (std_dev / 0.5))  # Normalize assuming 0.5 as max std_dev
        return consistency
    ...
    def get_learning_progress(self) -> Dict[str, Any]:
        ...
        recent_decisions = self.decisions[mid_point:] if mid_point > 0 else self.decisions
        
        early_avg = sum(d["decision_quality"] for d in early_decisions) / len(early_decisions) if early_decisions else 0
        recent_avg = sum(d["decision_quality"] for d in recent_decisions) / len(recent_decisions) if recent_decisions else 0
...
class TrainingExecutor(TradingExecutionInterface):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        try:
            from ..core.rag_system import MarketDataRAG, ChromaVectorStore, SentenceTransformerProvider
            if self.config.get("rag.enabled", False):
                vector_store = ChromaVectorStore(
                    persist_directory=self.config.get("rag.persist_directory", "./data/chroma_db")
                )
            ...
            warnings.warn(f"RAG system not available for training mode: {str(e)}")
    ...
    def end_training_session(self) -> Dict[str, Any]:
        ...
        feedback = self.get_training_feedback()
        ...
        report = {
            ...
            "trade_history": self.paper_executor.export_trade_history().to_dict("records"),
            "ready_for_live": self.confidence_metrics.is_ready_for_live_trading(
                threshold=self.config.get("training.confidence_threshold", 0.75)
            )
        }
    ...
    def place_order(self, order: OrderRequest) -> OrderResult:
        ...
        if self.current_session and self.current_session.is_active and result.status == OrderStatus.FILLED:
            # Analyze decision
            ...
            action = AgentAction(
                ...
                parameters={"symbol": order.symbol, "quantity": order.quantity, "type": order.order_type.value},
                result=result,
    ...
    def analyze_decision(self, order: OrderRequest, result: OrderResult) -> DecisionAnalysis:
        """Analyze a trading decision for learning purposes."""
        # Get market drivers
        market_drivers = []
        if self.analyze_market_drivers:
            market_drivers = self.market_driver_analysis.analyze_market_drivers(result.symbol, result)
        ...
        educational_context = ""
        if self.analyze_market_drivers:
            educational_context = self.market_driver_analysis.generate_educational_context(
    ...
    def _calculate_decision_quality(self, order: OrderRequest, result: OrderResult) -> float:
    ...
    def _generate_alternatives(self, order: OrderRequest, result: OrderResult) -> List[str]:
    ...
    def _calculate_confidence(self, decision_quality: float, risk_assessment: str) -> float:
    ...
    def get_training_feedback(self) -> TrainingFeedback:
        """Get comprehensive training feedback."""
        if not self.current_session:
            raise ValueError("No active training session")
        
        # Get recent decision analyses
        recent_analyses = self.decision_history[-10:] if len(self.decision_history) > 10 else self.decision_history
        
        # Get recent mistakes
        recent_mistakes = self.mistake_tracker.get_mistake_history()[-5:] if len(self.mistake_tracker.mistakes) > 5 else self.mistake_tracker.get_mistake_history()
        
        # Calculate scores
        confidence_score = self.confidence_metrics.calculate_confidence_score()
        consistency_score = self.confidence_metrics.calculate_consistency_score()
        risk_management_score = self.confidence_metrics.calculate_risk_management_score()
        ...
        if confidence_score < 0.5:
            recommendations.append("Practice more to build confidence in trading decisions")
    ...
    def export_training_report(self) -> Dict[str, Any]:
        ...
        if not self.current_session:
            raise ValueError("No training session to export")
        
        return self.end_training_session()
    ...
    def get_order_history(
        ...
        """Get order history."""
        return self.paper_executor.get_order_history(symbol, status, start_date, end_date, limit)
