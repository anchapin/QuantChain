...
class PerformanceMetrics:
    ...
    def update_metrics(self, trade_pnl: float, equity_curve: List[float]) -> None:
        ...
        if equity_curve and len(equity_curve) > 1:
            self.total_return = ((equity_curve[-1] - equity_curve[0]) / equity_curve[0]) * 100
...
class PaperTradingExecutor(TradingExecutionInterface):
        ...
        try:
            ...
            if order.side == OrderSide.BUY:
                if self.cash < total_cost:
                    raise ExecutionError(f"Insufficient funds: need {total_cost:.2f}, have {self.cash:.2f}")
                ...
                if order.symbol in self.positions:
                    ...
                    new_cost_basis = (pos.avg_entry_price * pos.quantity) + (fill_price * order.quantity)
    ...
    def cancel_order(self, order_id: str) -> OrderResult:
        ...
        if order.status not in [OrderStatus.PENDING, OrderStatus.PARTIALLY_FILLED]:
            raise ExecutionError(f"Cannot cancel order {order_id} with status {order.status}")
    ...
    def get_positions(self) -> List[Position]:
        ...
        for symbol, pos in self.positions.items():
            ...
            if abs(pos.quantity) > 1e-10 and pos.avg_entry_price > 0:
                pos.unrealized_pnl = (current_price - pos.avg_entry_price) * abs(pos.quantity)
                pos.unrealized_pnl_percent = ((current_price - pos.avg_entry_price) / pos.avg_entry_price) * 100
            else:
