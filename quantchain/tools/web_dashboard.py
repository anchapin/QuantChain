...
class DashboardCharts:
    ...
    def create_line_chart(
        self, x_data: List, y_data: List, title: str = "Chart"
    ) -> Optional[Any]:
        """Create a line chart."""
        if not HAS_PLOTLY:
    ...
    def create_performance_chart(
        self, returns: pd.Series, title: str = "Performance"
    ) -> Optional[Any]:
        ...
        fig.update_layout(
            title=title, template=self.theme, yaxis_title="Cumulative Return"
        )
        return fig
...
def create_dashboard(
    config: Optional[Config] = None, include_plots: bool = True
) -> Dict[str, Any]:
