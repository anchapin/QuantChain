...
class ChartRenderer:
    ...
    def render_candlestick_chart(
        ...
        if mpf is None or plt is None:
            raise QuantChainError("matplotlib and mplfinance required for chart rendering")
...
class PatternRecognizer:
    ...
    def _create_analysis_prompt(
        ...
        prompt = f"""
        Analyze the provided financial chart for {chart_image.symbol} on {chart_image.timeframe} timeframe.
        
        Please identify and analyze the following chart patterns if present:
        {enabled_patterns}
        
        Technical indicators applied: {", ".join(chart_image.indicators_applied)}
    ...
    def _calculate_signals(
        ...
        for pattern in patterns:
            if pattern.pattern_type in ["head_and_shoulders", "double_top", "descending_triangle"]:
                bearish_count += pattern.confidence
            elif pattern.pattern_type in ["inverse_head_and_shoulders", "double_bottom", "ascending_triangle"]:
        ...
        confluence = (max(bullish_count, bearish_count) / total_signals * 100) if total_signals > 0 else 0
    ...
    def _calculate_stop_loss(self, data: OHLCVData, patterns: List[Pattern]) -> Optional[float]:
    ...
    def _calculate_take_profit(self, data: OHLCVData, patterns: List[Pattern]) -> List[float]:
        ...
        if not profit_levels:
            ...
            if stop_loss:
                ...
                profit_levels.append(last_price + risk * self.config.default_take_profit_ratio)
        
        return profit_levels
    
    def _calculate_confidence(self, patterns: List[Pattern], confluence_score: float) -> float:
...
class ChartReaderAgent(QuantChainAgent):
    ...
    def __init__(
        ...
        self.pattern_recognizer = PatternRecognizer(self.agent_config, self.llm_provider)
    ...
    def _get_ohlcv_data(self, symbol: str, timeframe: str, limit: int = 200) -> Optional[OHLCVData]:
        """Get OHLCV data from the connector."""
        try:
            # Convert timeframe to connector format
            # This is a simplified mapping - actual implementation would vary by connector
            connector_timeframe = {
                "1m": "1min",
                "5m": "5min",
                "15m": "15min",
        ...
        for indicator_name in self.agent_config.indicators_enabled:
            try:
                if indicator_name == "SMA":
                    indicators.append(self.indicator_calculator.calculate_sma(data, period=20))
                elif indicator_name == "EMA":
                    indicators.append(self.indicator_calculator.calculate_ema(data, period=20))
                elif indicator_name == "RSI":
                    indicators.append(self.indicator_calculator.calculate_rsi(data, period=14))
    ...
    def generate_trading_recommendation(
        ...
        bullish_count = sum(1 for a in analyses.values() if a.recommended_action == "BUY")
        bearish_count = sum(1 for a in analyses.values() if a.recommended_action == "SELL")
        ...
        return {
            ...
            "reasoning": f"Based on {bullish_count} bullish and {bearish_count} bearish signals. Best analysis from {best_timeframe} timeframe: {best_analysis.reasoning}",
            "timeframe_analyses": {
                tf: {
