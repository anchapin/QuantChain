"""Smart Contract Auditor Agent for security and financial analysis of smart contracts."""
...
from ..core.agent_engine import QuantChainAgent, AgentState
...
class ContractRetriever:
    ...
    def get_contract_source(self, address: str, chain: str) -> ContractSource:
        ...
        try:
            ...
            if data['status'] != '1':
                raise QuantChainError(f"Error retrieving source code: {data.get('message', 'Unknown error')}")
            ...
            if creation_data['status'] == '1':
                ...
                creation_date = datetime.fromtimestamp(int(creation_result['txns'][0]['timeStamp']))
            ...
            return ContractSource(
                ...
                abi=abi,
                bytecode=bytes.fromhex(result.get('ByteCode', '')[2:]) if result.get('ByteCode') else b'',
                compiler_version=result.get('CompilerVersion', ''),
                optimization_enabled=result.get('OptimizationUsed') == '1',
                constructor_arguments=result.get('ConstructorArguments', ''),
                contract_type=self._detect_contract_type(result.get('SourceCode', ''), abi),
                verification_status=result.get('SourceCode') != 'Contract source code not verified',
                creation_date=creation_date,
                deployer_address=deployer,
                import_paths=self._extract_imports(result.get('SourceCode', '')),
                inherited_contracts=self._extract_inheritance(result.get('SourceCode', ''))
            )
    ...
    def _detect_contract_type(self, source_code: str, abi: List[Dict[str, Any]]) -> str:
        ...
        if any(event.get('type') == 'event' and 'Transfer' in event.get('name', '') for event in abi):
...
class VulnerabilityScanner:
    ...
    def _load_vulnerability_patterns(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                'recommendation': 'Avoid suicide/selfdestruct, use delegatecall carefully',
                'cvss_base': 8.2
            },
            {
                'id': 'tx_origin',
    ...
    def _check_pattern(self, source_code: str, pattern: Dict[str, Any]) -> List[Vulnerability]:
    ...
    def _check_access_control(self, contract: ContractSource) -> List[Vulnerability]:
        ...
        critical_functions = ['withdraw', 'mint', 'burn', 'transferOwnership', 'pause', 'unpause']
                ...
                if f'function {func}(' in line and 'public' in line:
                    ...
                    if not has_modifier:
                        vulnerabilities.append(Vulnerability(
                            ...
                            severity="HIGH",
                            description=f"Critical function {func} is public without access control",
                            location=f"Line {i + 1}",
                            code_snippet=line,
                            recommendation=f"Add access control modifier to {func} function",
                            cwe_id="CWE-284",
                            cvss_score=7.5,
                            confidence=85.0
                        ))
        ...
        if not safe_math_imported and not using_solidity_08:
            ...
            for i, line in enumerate(lines):
                if re.search(arithmetic_pattern, line):
                    ...
                    if not any(safe in line for safe in ['SafeMath', 'add(', 'sub(', 'mul(', 'div(']):
                        vulnerabilities.append(Vulnerability(
                            vulnerability_type="Arithmetic Overflow/Underflow",
                            severity="MEDIUM",
                            description="Potential arithmetic overflow/underflow vulnerability",
                            location=f"Line {i + 1}",
                            code_snippet=line,
    ...
    def _check_reentrancy(self, contract: ContractSource) -> List[Vulnerability]:
        ...
        for i, line in enumerate(lines):
            if any(call_pattern in line for call_pattern in ['.call(', '.transfer(', '.send(']):
                # Check if state changes happen after
                for j in range(i, min(i + 5, len(lines))):
                    next_line = lines[j]
                    if any(state_change in next_line for state_change in ['balance[', 'totalSupply', 'allowance']):
                        vulnerabilities.append(Vulnerability(
    ...
    def _calculate_gas_efficiency_score(self, contract: ContractSource) -> float:
        ...
        lines = contract.source_code.split('\n')
        loop_count = sum(1 for line in lines if any(pattern in line for pattern in loop_patterns))
    ...
    def _calculate_code_quality_score(self, contract: ContractSource, vulnerabilities: List[Vulnerability]) -> float:
    ...
    def _determine_audit_status(self, vulnerabilities: List[Vulnerability], security_score: float) -> str:
        """Determine overall audit status."""
        
        critical_count = sum(1 for v in vulnerabilities if v.severity == "CRITICAL")
        high_count = sum(1 for v in vulnerabilities if v.severity == "HIGH")
        
        if critical_count > 0 or high_count > self.config.high_vulnerabilities_threshold:
            return "DANGEROUS"
    ...
    def _generate_summary(self, vulnerabilities: List[Vulnerability], security_score: float) -> str:
        ...
        for severity in ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW']:
            if severity in severity_counts:
                summary_parts.append(f"{severity_count} {severity} issue(s)")
    ...
    def _generate_recommendations(self, vulnerabilities: List[Vulnerability]) -> List[str]:
        ...
        for vuln in vulnerabilities:
            ...
            if vuln_type not in rec_by_type:
                rec_by_type[vuln_type] = set()
            rec_by_type[vuln_type].add(vuln.recommendation)
        ...
        critical_count = sum(1 for v in vulnerabilities if v.severity == "CRITICAL")
        if critical_count > 0:
            recommendations.insert(0, "CRITICAL: Address critical vulnerabilities before deployment")
...
class FinancialAnalyzer:
    ...
    def analyze_tokenomics(self, contract_address: str, chain: str) -> TokenomicsAnalysis:
        ...
        # Simplified implementation - in production, would use APIs like Covalent, Moralis
...
class SmartContractAuditorAgent(QuantChainAgent):
    ...
    def audit_contract(
        ...
        try:
            ...
            vulnerability_report = self.vulnerability_scanner.scan_vulnerabilities(contract)
            ...
            if financial_analysis:
                if contract.contract_type == "ERC20":
                    tokenomics = self.financial_analyzer.analyze_tokenomics(address, chain)
                else:
                    protocol_analysis = self.financial_analyzer.analyze_defi_protocol(address, chain)
    ...
    def _generate_investment_recommendation(
        ...
        if tokenomics:
            financial_score = (
                ...
                min(100, (tokenomics.circulating_supply / tokenomics.total_supply * 100) * 0.3) +
                min(100, (1 - max(0, tokenomics.market_cap - 10000000) / 10000000) * 100 * 0.3)
            )
        elif protocol_analysis:
            financial_score = (
                min(100, protocol_analysis.sustainability_score * 0.5) +
                min(100, (1 - max(0, protocol_analysis.total_value_locked - 100000000) / 100000000) * 100 * 0.3) +
                min(100, (1 - max(0, max(protocol_analysis.apy_rates.values() or [0]) - 30) / 30) * 100 * 0.2)
            )
        ...
        if vulnerability_report.vulnerabilities:
            risk_factors.append(f"{len(vulnerability_report.vulnerabilities)} security issues found")
        ...
        if tokenomics and tokenomics.market_cap > 0:
            ...
            potential_return = max(0, (100 - security_score) / 100 * 50 + financial_score / 100 * 30)
        ...
        return InvestmentRecommendation(
            ...
            reasoning=f"Based on security score of {security_score:.1f} and financial score of {financial_score:.1f}",
