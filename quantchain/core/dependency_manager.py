...
class DependencyManager:
    ...
    def require(self, dependency: str, message: Optional[str] = None) -> bool:
        ...
        if not self.is_available(dependency):
            error_msg = message or f"Required dependency '{dependency}' is not available"
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if not manager.is_available(dependency):
                if fallback:
                    return fallback
                raise ImportError(f"Optional dependency '{dependency}' is required for {func.__name__}")
